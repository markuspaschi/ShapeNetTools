...
class Model(object):
    ...
    def build(self):
        ...
        # Build metrics for eval purposes!
        # we dont minimize any loss here
        self._loss()
    ...
    def load(self, sess=None, start_time=None):
        if not sess:
            raise AttributeError("TensorFlow session not provided.")
        if not start_time:
        ...
        save_path = "Data/checkpoint/%s/%s.ckpt" % (start_time, self.name)
...
#updata image feature
        ...
        # add loss here for eval purposes!
