...
parser = argparse.ArgumentParser(description='Enter additional args, --training_date is mandatory')
parser.add_argument('--training_date', type=str, required=True, help='Training date (corresponds to folder in log/ or Data/checkpoint where the training data is stored).')
parser.add_argument('--data_list', type=str, required=True, help='Data list (corresponds to folder in Data/train_test_lists/).')
args = parser.parse_args()

training_date = args.training_date

dir = 'Data/checkpoint/%s'% training_date
if not os.path.exists(dir):
    sys.exit("INVALID --training_date")

DATA_LIST = args.data_list

if not os.path.exists(DATA_LIST):
    sys.exit("INVALID --data_list")

# Settings
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('data_list', DATA_LIST, 'Data list path.')
...
log = open('log/%s/record_evaluation.txt'%(training_date), 'a')
